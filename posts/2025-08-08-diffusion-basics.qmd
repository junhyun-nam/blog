...
Real data distribution으로 부터 샘플링한 data point $\mathbf{x}_0 \sim q(\mathbf{x})$에 대해 forward diffusion process는 T step 동안 작은 양의 Gaussian noise를 더하는 것으로 정의 됨. 이에 따라 noisy sample의 sequence $\mathbf{x}_1, \cdots, \mathbf{x}_T$가 정의 되며, 매 step 마다 더해지는 Gaussian noise의 양은 variance schedule $\{\beta_t \in (0, 1) \}_{t=1}^T$를 따름.
